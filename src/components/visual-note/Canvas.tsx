...
export function Canvas() {
	...
	useEffect(() => {
		...
		const handler = (e: WheelEvent) => {
			...
			setZoom(nextZoom);
			setOffset(
				mouseX - scale * (mouseX - o.x),
				mouseY - scale * (mouseY - o.y),
			);
		};
	...
	const onMouseDown = useCallback(
		(e: React.MouseEvent) => {
			...
			// background click → deselect (node clicks stop propagation)
			...
			>
				...
				{nodes.map((node) => (
					<NoteNodeCard
						...
						onInputPortEnter={(id) =>
							isDraggingEdge.current && setHoveredInputId(id)
						}
				...
				<AlertDialogContent>
					...
					<AlertDialogFooter>
						...
						<AlertDialogAction
							...
							onClick={() => {
								if (pendingDeleteId) removeNode(pendingDeleteId);
